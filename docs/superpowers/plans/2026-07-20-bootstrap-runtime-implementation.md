...
Use `psycopg_pool.AsyncConnectionPool`; assign event sequence inside a transaction while locking the tenant-scoped trace row. `start_span`, `append_event`, and `finish_span` acquire that same trace-row lock and require `status = 'running'` with `finished_at IS NULL`, so they serialize against `finish_trace` and cannot mutate a terminal trace. All trace/span write queries include both the object ID and `tenant_id`; `start_span` validates a supplied parent belongs to the same trace and tenant inside the lock transaction, and `append_event` applies the same rule to its span. `finish_span` accepts only `completed`, `failed`, `cancelled`, or `skipped`, updates only an unfinished span, and distinguishes missing, already-finished, and terminal-trace failures. `finish_trace` updates only rows whose `finished_at IS NULL`, distinguishes a missing trace from an already-finalized trace, and never lets a second terminal write clobber the first outcome. `get_trace` must include `WHERE id = %s AND tenant_id = %s`. Store typed event payloads as JSONB and never accept secrets in repository APIs.
